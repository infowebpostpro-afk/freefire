/**
 * Free Fire Girls Nickname Browser - Application Controller
 ...
 * lightweight customizer, comparison, pick-for-me, roulette, and ambient starlight.
 */

const GirlsApp = {
  state: {
    activeCategory: 'queen',
    activeVibe: 'all',
    activePlaystyle: 'all',
    activeLength: 'any',
    activeWordCount: 'any',
    ...
    // Persistent Storage
    saved: JSON.parse(localStorage.getItem('ff_girls_saved') || '[]'),
    shortlist: JSON.parse(localStorage.getItem('ff_girls_shortlist') || '[]'),
    recents: JSON.parse(localStorage.getItem('ff_girls_recents') || '[]'),
    copied: JSON.parse(localStorage.getItem('ff_girls_copied') || '[]'),
    
    // Active Modal Context
    activeNameObj: null,
    previewStyleLevel: 'wings', // 'clean' | 'wings' | 'royal' | 'extreme'
    previewContextTab: 'profile',
  ...
  applyFilters(resetPagination = true) {
    ...
    this.state.currentResults = GirlsEngine.filterNames({
  ...
  attachCardEventListeners(grid) {
    ...
    grid.querySelectorAll('.btn-card-fav').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        this.toggleSave(btn.dataset.id);
      });
    });
    ...
    grid.querySelectorAll('.btn-card-shortlist').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        this.toggleShortlist(btn.dataset.id);
      });
    });
  ...
  initSearch() {
    const searchInput = document.getElementById('girls-search-input');
    ...
    document.addEventListener('click', (e) => {
      if (!e.target.closest('.girls-search-section') && dropdown) {
  ...
  initActionButtons() {
    ...
    if (btnShuffle) {
      btnShuffle.addEventListener('click', () => {
        this.state.currentResults = GirlsEngine.shuffle(this.state.currentResults);
        this.renderNicknameGrid();
    ...
    if (btnSurprise) {
      btnSurprise.addEventListener('click', () => {
        ...
        this.state.currentResults = GirlsEngine.surpriseMe(16);
        this.renderActiveFilterChips();
        this.renderNicknameGrid();
        this.showToast('🎲 Fresh surprise selection!');
      });
    }
    ...
    const sortSelect = document.getElementById('girls-sort-select');
    if (sortSelect) {
  ...
  renderActiveFilterChips() {
    ...
    if (this.state.activeCategory !== 'all') {
      const cat = GIRLS_CATEGORIES.find(c => c.id === this.state.activeCategory);
    ...
    container.querySelectorAll('.chip-remove-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (type === 'search') {
          this.state.searchQuery = '';
          const sInput = document.getElementById('girls-search-input');
  ...
  triggerMoreLikeThis(id) {
    const related = GirlsEngine.getMoreLikeThis(id, 8);
    const target = GIRLS_DATABASE.find(n => n.id === id);
    if (!target) return;
  ...
  openPreviewModal(item) {
    ...
    this.state.previewStyleLevel = 'wings';
  ...
  updatePreviewModalContent() {
    ...
    if (remixContainer) {
      const remixes = generateGirlsRemixes(item.baseName);
      let html = '';
      remixes.forEach(r => {
  ...
  getStyledNameForLevel(baseName, level) {
    switch (level) {
      case 'clean': return GIRLS_STYLE_RULES.clean(baseName);
      case 'royal': return GIRLS_STYLE_RULES.royal(baseName);
      case 'heart': return GIRLS_STYLE_RULES.heart(baseName);
      case 'extreme': return GIRLS_STYLE_RULES.extreme(baseName);
      case 'wings':
      default: return GIRLS_STYLE_RULES.wings(baseName);
    }
  },
  ...
  openCustomizeModal(item) {
    ...
    this.state.customizerLeft = '𓆩';
    this.state.customizerRight = '𓆪';
    this.state.customizerStyle = 'wings';
  ...
  updateCustomizerPreview() {
    ...
    const base = input.value.trim() || 'Queen';
    ...
    if (alertBox) {
      const hasUnicode = GirlsEngine.hasSpecialCharacters(full);
  ...
  toggleSave(id) {
    ...
    localStorage.setItem('ff_girls_saved', JSON.stringify(this.state.saved));
    this.updateBadges();
    this.renderNicknameGrid();
  ...
  toggleShortlist(id) {
    ...
    localStorage.setItem('ff_girls_shortlist', JSON.stringify(this.state.shortlist));
    this.updateBadges();
    this.renderNicknameGrid();
    ...
    if (this.state.shortlist.length === 0) {
      body.innerHTML = `
        <div style="text-align: center; padding: 3rem 1rem; color: var(--text-muted);">
          ...
          <p style="font-size: 0.85rem;">Add names you're deciding between, compare them, or share with friends.</p>
        </div>
      `;
      return;
    }
  ...
  addToRecents(item) {
    ...
    localStorage.setItem('ff_girls_recents', JSON.stringify(this.state.recents));
  },
    ...
    candidates.forEach(item => {
      ...
      html += `
        <div class="nickname-card" style="margin-bottom: 0;">
          <div class="card-nickname-text" style="font-size: 1.3rem; margin-bottom: 0.5rem; color: #ff7597;">
  ...
  executePickForMe() {
    ...
    const vibe = document.querySelector('.btn-pick-vibe.active')?.dataset.vibe || 'queen';

    const results = GirlsEngine.pickForMe(ps, vibe, 8);
  ...
  spinRoulette(display, spinBtn) {
    ...
    const candidates = GirlsEngine.shuffle(GIRLS_DATABASE);
    let index = 0;
    let iterations = 0;
    ...
    const timer = setInterval(() => {
      ...
      display.textContent = GIRLS_STYLE_RULES.wings(item.baseName);
      ...
      if (iterations >= maxIterations) {
        ...
        const winnerStyled = GIRLS_STYLE_RULES.wings(winner.baseName);
  ...
  initModalsAndDrawers() {
    document.querySelectorAll('.modal-close-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        document.querySelectorAll('.girls-modal-overlay, .girls-drawer-overlay').forEach(el => {
          el.classList.remove('active');
        });
      });
    });

    document.querySelectorAll('.girls-modal-overlay').forEach(overlay => {
      overlay.addEventListener('click', (e) => {
        if (e.target === overlay) overlay.classList.remove('active');
      });
    });

    const drawerOverlay = document.getElementById('girls-drawer-overlay');
    if (drawerOverlay) {
    ...
    if (btnCopyShortlistAll) {
      btnCopyShortlistAll.addEventListener('click', () => {
        ...
        const text = this.state.shortlist.map((item, i) => `${i + 1}. ${GIRLS_STYLE_RULES.wings(item.baseName)}`).join('\n');
    ...
    if (btnShareShortlist) {
      btnShareShortlist.addEventListener('click', () => {
        ...
        const text = `Help me choose my Free Fire nickname:\n` + 
          this.state.shortlist.map((item, i) => `${i + 1}. ${GIRLS_STYLE_RULES.wings(item.baseName)}`).join('\n');
        if (navigator.share) {
  ...
  openDrawer(type) {
    const overlay = document.getElementById('girls-drawer-overlay');
  ...
  initPreviewModalEvents() {
    ...
    if (btnCleaner) {
      btnCleaner.addEventListener('click', () => {
        if (this.state.previewStyleLevel === 'extreme') this.state.previewStyleLevel = 'royal';
        else if (this.state.previewStyleLevel === 'royal') this.state.previewStyleLevel = 'wings';
        else if (this.state.previewStyleLevel === 'wings') this.state.previewStyleLevel = 'clean';
        this.updatePreviewModalContent();
      });
    }
    ...
    if (btnBolder) {
      btnBolder.addEventListener('click', () => {
        if (this.state.previewStyleLevel === 'clean') this.state.previewStyleLevel = 'wings';
        else if (this.state.previewStyleLevel === 'wings') this.state.previewStyleLevel = 'royal';
        else if (this.state.previewStyleLevel === 'royal') this.state.previewStyleLevel = 'extreme';
        this.updatePreviewModalContent();
      });
    }
  ...
  initCustomizerEvents() {
    ...
    document.querySelectorAll('.btn-frame-preset').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        btn.classList.add('active');
        this.applyCustomizerFrame(btn.dataset.frame);
      });
    });
    ...
    if (symbolsWrap) {
      let html = '';
      GIRLS_POPULAR_SYMBOLS.forEach(sym => {
  ...
  applyCustomizerFrame(frameId) {
    switch (frameId) {
      ...
      case 'heart':
        ...
        break;
      case 'butterfly':
        this.state.customizerLeft = 'ʚ';
        this.state.customizerRight = 'ɞ';
        break;
      case 'blossom':
        this.state.customizerLeft = '✿';
        this.state.customizerRight = '✿';
  ...
  initMobileBottomNav() {
    ...
    if (btnSearch) {
      btnSearch.addEventListener('click', () => {
        const input = document.getElementById('girls-search-input');
        if (input) {
  ...
  onCopySuccess(text, triggerBtn, showFullToast, customMsg) {
    ...
    localStorage.setItem('ff_girls_copied', JSON.stringify(this.state.copied));
  ...
  showToast(msg) {
    const toast = document.getElementById('girls-toast');
    if (!toast) return;
  ...
  /* ===================================================================
     AMBIENT STARLIGHT PARTICLES BACKGROUND
     =================================================================== */

  initAmbientStarlight() {
    ...
    for (let i = 0; i < count; i++) {
      particles.push({
        ...
        color: Math.random() > 0.4 ? '#ff2a5f' : '#b388ff'
      });
    }
